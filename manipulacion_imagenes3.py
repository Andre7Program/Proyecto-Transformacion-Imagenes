...
def cargar_imagen(ruta):
    """Carga una imagen desde la ruta especificada, manteniendo transparencia si está presente."""
    imagen = cv2.imread(ruta, cv2.IMREAD_UNCHANGED)
    if imagen is None:
        raise FileNotFoundError("No se pudo cargar la imagen. Verifica la ruta.")
    if imagen.shape[2] == 3:  # Agregar canal alfa si no existe
        imagen = cv2.cvtColor(imagen, cv2.COLOR_BGR2RGB)
        imagen = np.dstack((imagen, np.full((imagen.shape[0], imagen.shape[1]), 255, dtype=np.uint8)))
    return imagen

def rotar_imagen(imagen, angulo):
    """Aplica una rotación a la imagen con un fondo transparente."""
    filas, columnas, _ = imagen.shape

    # Agregar canal alfa para transparencia
    if imagen.shape[2] == 3:  # Si la imagen no tiene canal alfa
        imagen = np.dstack((imagen, np.full((filas, columnas), 255, dtype=np.uint8)))

    matriz_rotacion = cv2.getRotationMatrix2D((columnas / 2, filas / 2), angulo, 1)

    # Expandir el tamaño del canvas para evitar recortes
    cos = np.abs(matriz_rotacion[0, 0])
    sin = np.abs(matriz_rotacion[0, 1])
    nuevo_ancho = int(filas * sin + columnas * cos)
    nuevo_alto = int(filas * cos + columnas * sin)

    # Ajustar la matriz de rotación al nuevo tamaño
    matriz_rotacion[0, 2] += (nuevo_ancho - columnas) / 2
    matriz_rotacion[1, 2] += (nuevo_alto - filas) / 2

    imagen_rotada = cv2.warpAffine(imagen, matriz_rotacion, (nuevo_ancho, nuevo_alto), borderValue=(0, 0, 0, 0))
...
def trasladar_imagen(imagen, desplazamiento_x, desplazamiento_y):
    """Aplica una traslación a la imagen con un fondo transparente."""
    filas, columnas, _ = imagen.shape
    ...
    imagen_trasladada = cv2.warpAffine(imagen, matriz_traslacion, (columnas, filas), borderValue=(0, 0, 0, 0))
...
def guardar_imagen():
    """Guarda la imagen transformada con transparencia."""
    if imagen_transformada is None:
        messagebox.showerror("Error", "No hay una imagen transformada para guardar.")
        return

    ruta = filedialog.asksaveasfilename(defaultextension=".png", filetypes=[("Imágenes PNG", "*.png")])
    if ruta:
        cv2.imwrite(ruta, cv2.cvtColor(imagen_transformada, cv2.COLOR_RGBA2BGRA))  # Convertir a BGRA para guardar
